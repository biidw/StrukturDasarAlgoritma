#include <iostream>
#include <windows.h> 						// menggunakan gotoxy, GetAsyncKeyState dan beberapa fungsi lainnya // Awal pendeklarasian beberapa konstanta
#define batas_atas 2 						// batas atas game
#define batas_bawah 23 						// batas bawah game
#define batas_samping 2 					// batas samping kiri game
#define batas_samping_k 78 					// batas samping kanan game // Akhir pendeklarasian konstanta
...
void gotoxy(int x, int y) {					// memindahkan kursor pada console
...
void tampil() {
 ...
 cout << char(2); 							// untuk menggambar gambar kepala karena ascii dari smiley tersebut adalah 2
 
 gotoxy(makX, makY); 					 // nilai makX dan makY adalah random // digunakan untuk posisi makanan, jadi posisi makanan random
 cout << "*"; 								// gambar makanan
...
void velo() {								// fungsi untuk melakukan update posisi snake sesuai tombol yang ditekan
 ganti_posisi();
 hapus();
 iX += veloX;
 iY += veloY;
...
bool isDestroy() {														 // selain untuk meng-cek apakah snake sudah kalah fungsi ini juga menghapus kepala snake yang tersisa jika membentur dinding 
 ...
 if(iY == batas_bawah) { iY = 3; gotoxy(iX, 23); cout << " "; }				 // trace();
...
void cBorder() {											 // fungsi untuk membuat garis tepi game
 ...
 for(int i = 1; i <= 24; i++) {							// akhir pembuatan garis tepi, penulisan skor
...
void skor() {											// Untuk menulis skor terbaru dan panjang dari snake
...
void tulis_skor() {										// Fungsi yang menjalankan beberapa fungsi yang berjalan // pada awal program, hanya sekali
...
void inisialisasi() {									// output program
...
int main() {
 ...
 while(!(isDestroy())) {								// selama snake belum rusak atau kalah
   ...
   getkey(); 											// untuk mendapatkan tombol apa yang ditekan user dan menjalankan beberapa fungsi yang diperlukan
   
   if(isEaten()) {										// meng-cek apakah makanan telah dilalap oleh snake
